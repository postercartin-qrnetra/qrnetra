...
export async function generateStaticParams() {
  return getProductsByCategory("assets").map((product) => ({ slug: product.slug }));
}
...
export default async function AssetProductPage({ params }: Props) {
  const { slug } = await params;
  const product = getProduct(slug);
  if (!product || product.category !== "assets") notFound();
