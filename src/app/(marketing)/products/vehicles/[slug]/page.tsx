...
export async function generateStaticParams() {
  return getProductsByCategory("vehicles").map((product) => ({ slug: product.slug }));
}
...
export default async function VehicleProductPage({ params }: Props) {
  const { slug } = await params;
  const product = getProduct(slug);
  if (!product || product.category !== "vehicles") notFound();
