...
export async function generateStaticParams() {
  return getProductsByCategory("kids").map((product) => ({ slug: product.slug }));
}
...
export default async function KidProductPage({ params }: Props) {
  const { slug } = await params;
  const product = getProduct(slug);
  if (!product || product.category !== "kids") notFound();
